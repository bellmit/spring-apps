package com.haozhuo.datag.model.report;

import lombok.Getter;
import lombok.Setter;

@Getter
@Setter
public class RepAbnormal {

    private String bmi;
    private String diya;
    private String gaoya;
    private String xuetang;
    private String tanghuaxuehongdanbai;
    private String jiazhuangxianjiejie;
    private String ruxianjiejie;
    private String feijiejie;
    private String ganzangjiejie;
    private String weixirou;
    private String changdaoxirou;
    private String luanchaolangzhong;
    private String gongjingtct;
    private String gongjinghpv;
    private String gangongnengalt;
    private String gangongnengast;
    private String yiganbiaomiankangyuan;
    private String yiganbiaomiankangti;
    private String yiganekangyuan;
    private String yiganekangti;
    private String yiganhexinkangti;
    private String yigandna;
    private String yiganqians1kangyuan;
    private String bingganbingdukangti;
    private String bingganbingdurna;
    private String ganyinghuachaosheng;
    private String xinzangchaoshengyichang;
    private String xindiantu;



}
