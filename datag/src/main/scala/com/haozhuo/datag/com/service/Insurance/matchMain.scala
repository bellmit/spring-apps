package com.haozhuo.datag.com.service.Insurance

import scala.util.matching.Regex
import scala.collection.mutable._
import scala.util.control.Breaks._

/*
匹配的主函数入口
先通过非数值匹配，匹配到了返回0(含有重大异常)，否则返回1
返回1后执行数值匹配

数值匹配：匹配含有异常项,提取数据

*/


object matchMain {

  //非数值匹配
  def noNumMatch(rs_val:String) ={
    val regex = new Regex(
      """
        |心律失常|
        |呼吸(音|声)消失|呼吸(音|声)减弱|
        |重.*贫血貌|贫血貌.*重|
        |重度水肿|
        |急腹症|
        |急性.{0,10}青光眼|
        |(视网膜动脉硬化.{0,10}IV|IV.{0,10}视网膜动脉硬化)|
        |突发.{0,3}视力下降|
        |流行性.{0,10}出血性结膜炎|
        |喉头水肿|
        |活动性鼻出血|
        |大量气胸|
        |急性气胸|
        |张力性气胸|
        |液气胸|
        |脑疝|
        |脑.{0,10}出血|
        |硬膜下血肿.{0,10}急性|急性.{0,10}硬膜下血肿|
        |硬膜外血肿.{0,10}急性|急性.{0,10}硬膜外血肿|
        |严重.{0,5}颅脑挫裂伤|颅脑挫裂伤.{0,10}重|
        |蛛网膜下腔出血.{0,10}急性|蛛网膜下腔出血|
        |急性脑积水|
        |颅内.{0,10}急性.{0,10}大面积脑梗死|
        |严重.{0,5}脊柱损伤|脊柱损伤.{0,5}重|
        |(肺栓塞|肺梗死)|
        |大量.{0,10}胸腔积液|胸腔积液.{0,10}大量|
        |主动脉夹层|
        |食道.{0,10}异物|
        |消化道.{0,20}穿孔|
        |绞窄性.{0,3}肠梗阻|
        |急性.{0,3}胆道梗阻|
        |急性出血坏死性胰腺炎|
        |多发复合伤.{0,40}破裂|破裂.{0,20}多发复合伤|
        |外伤.{0,15}出血.{0,20}腹腔积液|
        |急性.*胆囊炎.*胆囊化脓.*穿孔|
        |胆囊颈部结石.*嵌顿|
        |急性.*坏死性胰腺炎|
        |(肝硬化|腹膜炎|肿瘤).*腹腔积液|
        |宫外孕|
        |卵巢(囊肿|肿瘤).*蒂扭转|卵.*蒂扭转|
        |卵巢(囊肿|肿瘤).*破裂|
        |黄体.{0,5}破裂|
        |大量心包积液.*心包填塞|心包积液.*大量.*心包填塞|
        |心.*(游离血栓|血栓游离)|
        |腹主动脉瘤|
        |下肢静脉.*(游离血栓|血栓游离)|
        |(急性|亚急性).*心肌梗|心肌梗.*(急性|亚急性)|
        |急性.*心肌缺血|心肌缺血.*急性|
        |心室扑动|心室颤动|
        |尖端扭转.{0,5}室性心动过速|室性心动过速.{0,5}尖端扭转|
        |多形性.{0,5}室性心动过速|室性心动过速.{0,5}多形性|
        |双向性.{0,5}室性心动过速|室性心动过速.{0,5}双向性|
        |严重.{0,5}低钾血症|低钾血症.{0,5}严重|
        |严重.{0,5}高钾血症|高钾血症.{0,5}严重|
        |急性.{0,5}肺栓塞|肺栓塞.{0,5}急性|
        |显性T波电交替|
        |多源性室性早搏.*RonT型室性早搏|
        |心肌炎|
        |心脏起搏器.*(感知功能障碍|感知功能不良)|
        |(食管|胃底).*重度静脉曲张|(食管|胃底).*静脉曲张.{0,5}重|
        |消化道.*活动性出血|
        |肝脾表面不平.*结节|
        |肝脾.{0,10}重度肿大|肝脾.{0,10}中度肿大|肝脾.{0,10}中度|肝脾.{0,10}重度|
        |腹水|
        |黄疸|
        |腹部扪及包块|腹部.*可扪及包块|
        |巨脾|
        |皮肤.*恶性肿瘤|恶性肿瘤.*皮肤|皮肤癌|癌|
        |恶性淋巴|转移性淋巴结肿大|
        |甲状腺癌|甲状腺.*恶性肿瘤|
        |乳腺癌|乳腺.*恶性肿瘤|
        |肛管癌|肛管.*恶性肿瘤|
        |直肠癌|直肠.*恶性肿瘤|
        |前列腺癌|前列腺.*恶性肿瘤|
        |睾丸.*恶性肿瘤|睾丸癌|
        |阴茎癌|阴茎.*恶性肿瘤|
        |恶性肿瘤|癌|
        |性病|
        |宫颈上皮.*病变|
        |外阴肿物|
        |宫颈肿物|
        |急性虹膜睫状体炎|
        |眶内肿瘤|眼眶.*肿瘤|
        |急性视神经炎|
        |急性.*缺血性视网膜病变|缺血性视网膜病变.*急性|
        |视网膜脱.*|
        |黄斑裂孔|
        |眼底静脉血栓|
        |眼.*动脉栓塞|
        |III.*视网膜动脉硬化|视网膜动脉硬化.*III|
        |视乳头水肿|
        |角膜炎|
        |耳聋|
        |鼻咽癌|
        |急性会厌炎|声门上喉炎|会厌前咽峡炎|
        |喉.*癌|
        |回缩性涕血|
        |口腔.*肿块|口腔.*肿瘤|
        |经久不愈.*慢性溃疡|慢性溃疡.*经久不愈|
        |天疱疮|
        |恶性肉芽肿|
        |外周血.*幼稚细胞|
        |白细胞分类.*严重异常|白细胞分类.*异常.*重|
        |痢疾|
        |不典型鳞状细胞|非典型鳞状细胞|
        |低级别鳞状上皮内病变|
        |高级别鳞状上皮内病变|
        |鳞状细胞癌|
        |不典型腺上皮细胞|非典型腺上皮细胞|
        |腺原位癌|
        |腺癌|
        |肺部.*占位性病变.*可疑肿块|肺部.*可疑肿块.*占位性病变|
        |肿瘤标志物.*阳性|
        |免疫缺陷病毒(HIV).*阳性|HIV.*阳性|免疫缺陷病毒.*阳性|艾滋病|
        |肺.*结节.*恶性|恶性.*肺.*结节|
        |中等量.*胸腔积液|胸腔积液.*中等量|
        |气胸|
        |肺不张|
        |肺空洞|
        |活动性肺结核|
        |纵隔淋巴结肿大|
        |纵隔.*占位性病变|
        |心脏.*明显增大|心脏.*增大明显|
        |心包积液|
        |膈下占位性病变|
        |骨质破坏|
        |骨转移灶|
        |乳腺肿块|
        |颅内占位性病变|
        |慢性硬膜下血肿|
        |颅.*血管(狭窄|闭塞)|脑.*血管(狭窄|闭塞)|
        |颅内动脉瘤|
        |颅内动静脉畸形|脑.*血管畸形|
        |椎管.*占位|
        |脑梗塞|
        |大片.{0,5}(肺实变|肺渗出性.*变)|
        |肝.*恶性肿瘤|
        |胆管囊腺瘤|
        |肝脏囊肿合并.{0,5}(感染|出血)|
        |胆囊胆管.*高度恶性病变|
        |(无蒂性|广基).*病变胆囊息肉|
        |胆囊息肉病变.*(变宽|增大)|
        |胆囊息肉.*病灶.*周围黏膜.*(浸润|增厚)|
        |胆囊占位.*恶性病变|恶性病变.*胆囊占位|
        |恶性病变.*胰腺占位|胰腺占位.*恶性病变|
        |恶性病变.*脾脏占位|脾脏占位.*恶性病变|
        |(肝|脾).*(中度|重度)肿大|
        |肝硬化.*伴门脉高压|
        |肝外.{0,5}梗阻|
        |腹膜后占位性病变|
        |膀胱.*恶性占位性病变|
        |前列腺.*恶性占位性病变|
        |子宫.*恶性占位性病变|
        |卵巢.*恶性占位性病变|
        |肾脏.*恶性占位性病变|
        |卵巢畸胎瘤|
        |动脉硬化.*斑块伴.*狭窄|
        |颈动脉.*闭塞|
        |动脉瘤|
        |甲状腺恶性肿瘤|
        |乳腺恶性肿瘤|
        |甲状腺实性结节.*沙砾样钙化|
        |乳腺实性结节.*沙砾样钙化|
        |严重.*瓣膜病|瓣膜病.*重|
        |左房粘液瘤|
        |心肌病|
        |严重.*先天性心脏病|先天性心脏病.*重|
        |心脏肿瘤|
        |(中度|重度).*(不|非)典型增生|
        |重度肠化|
        |(中|重)度.*黏膜.*萎缩|
        |腺瘤.{0,10}RADS.*(III|3)|
      """
    )


  val Regex=regex.findAllIn(rs_val)

    //遍历集合
    var vue="1"

    while (Regex.hasNext){

    vue=Regex.next()


    }

    if(vue.equals("1")){
      "1"
    }else{
      "0,"+vue
    }

  }

  //数值判断，从后往前
  def numMatch(rs_val:String)={
    //字符串的拉取
    val regex = new Regex(
      """
      |结肠溃疡.*[0-9]+.{0,10}.*息肉|结肠溃疡.*息肉.{0,10}[0-9]+.{0,10}|
      |十二指肠.{0,20}溃疡.{0,10}m.{7}|十二指肠.{0,30}溃疡|
      |胃.{0,20}溃疡|
      |乳腺.{0,3}实.{0,3}性占位.{0,35}|
      |甲状腺.{0,3}实.{0,3}性占位.{0,35}|
      |甲状腺.{0,3}混合.{0,3}性占位.{0,35}|
      |卵巢囊肿.{0,20}|
      |子宫内膜.{0,10}厚.{0,15}|
      |子宫肌瘤.{0,20}|
      |泌尿.{0,20}实性占位性病变|生殖系统.{0,20}实性占位性病变|
      |肾.{0,3}囊肿.{0,20}|
      |腹部.{0,20}占位性病变|
      |胰腺囊肿.{0,20}|
      |主胰管扩张.{0,30}胰腺囊肿|
      |胆囊.{0,20}息肉.{0,10}|
      |肝脏.{0,20}血管瘤.{0,20}|
      |肝脏.{0,20}囊肿.{0,20}|
      |颈.{0,3}动脉.{0,5}狭窄.{0,20}%|
      |血管瘤.{0,20}|
      |肝.{0,3}血管瘤.{0,20}|
      |血清肌酸激酶-MB同工酶(CK-MB)升高，肌钙蛋白:阳性，肌红蛋白:阳性|
      |宫颈刮片.{0,10}巴氏.{0,10}级|
      |子宫内膜.{0,20}m|

      |.{0,10}心室停搏.{0,10}|
      |室.{0,3}性心动过速.{0,20}心室率.{0,20}|

      |QT间期延长.{0,20}|
      |长RR间期.{0,20}|
      |严重心动过缓、高度及三度房室阻滞，平均心室率≤35次/min|
      |心房颤动伴心室预激，最短RR间期≤250 ms|




      """
    )


    //数值拉取
    def getCm(str:String)= {
      val regex_num = new Regex(
        """
          |[0-9]+\.[0-9]+|
          |[0-9]+|
        """
      )

      //定义返回值
      var max=" "

      //转换为cm,获取最大值
      if (str.contains("mm")|str.contains("MM")|str.contains("M M")|str.contains("m m")){


        //创建数组获取最大值
        val row_ints = new ArrayBuffer[Double]

        val result_1=regex_num.findAllIn(str)
        while(result_1.hasNext){
          //获取最大值
          row_ints.append(result_1.next().toDouble/10.0)
        }
         max= row_ints.max.toString
      }

      //不转换，获取最大值
      else if(str.contains("cm")|str.contains("CM")|str.contains("c m")|str.contains("C M")){
        //创建数组获取最大值
        val row_ints = new ArrayBuffer[Double]

        val result_1=regex_num.findAllIn(str)
        while(result_1.hasNext){
          //获取最大值
          row_ints.append(result_1.next().toDouble)
        }
         max= row_ints.max.toString
      }
      else{
        val row_ints = new ArrayBuffer[Double]
        val result_1=regex_num.findAllIn(str)
        while(result_1.hasNext){
          //获取最大值
          row_ints.append(result_1.next().toDouble)
        }
        //包含数字
        if(row_ints.isEmpty){
          println("没有匹配到数字")
          max="-1"
        }else{
          max=row_ints.max.toString
        }

      }



    max

    }



    //获取每条异常最大值与配置参数进行对比，返回结果为0（异常）
    var Regex=regex.findAllIn(rs_val)
    var result_status="1"

    var result_vue="正常"

    while (Regex.hasNext){

      val  string_next=Regex.next()



      /**这里不能用match(它没有container)
        * 判断大小，如果大小存在，不在进行循环直接跳出
        */
      if(string_next.contains("结肠")){
        //获取最大值
        val max: String = getCm(string_next)
        //判断
        result_status= if(max.toDouble>0.5) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>0.5) string_next else result_vue

      }

      if(string_next.contains("十二指肠")){
        //获取最大值
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>2.5) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>2.5) string_next else result_vue
      }

      if(string_next.contains("胃溃疡")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>3.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>3.0) string_next else result_vue
      }

      if(string_next.contains("乳腺")&string_next.contains("占位")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>2.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>2.0) string_next else result_vue
      }

      if(string_next.contains("甲状腺")&string_next.contains("实")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>2.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>2.0) string_next else result_vue
      }

      if(string_next.contains("甲状腺")&string_next.contains("混合性")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>2.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>2.0) string_next else result_vue
      }

      if(string_next.contains("卵巢")&string_next.contains("肿")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>5.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>5.0) string_next else result_vue
      }

      if(string_next.contains("子宫内膜")&(rs_val.contains("不规则阴道出血")|rs_val.contains("阴道不规则出血"))){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>0.5) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>0.5) string_next else result_vue
      }

      if(string_next.contains("子宫肌瘤")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>5.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>5.0) string_next else result_vue
      }

      if((string_next.contains("泌尿")||string_next.contains("生殖系统"))&string_next.contains("占位性病变")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>3.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>3.0) string_next else result_vue
      }

      if(string_next.contains("肾脏")&string_next.contains("肿")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>5.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>5.0) string_next else result_vue
      }

      if(string_next.contains("腹部")&string_next.contains("占位性病变")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>3.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>3.0) string_next else result_vue
      }

      if(string_next.contains("胰腺")&string_next.contains("囊肿")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>3.0) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>3.0) string_next else result_vue
      }

      if(string_next.contains("主胰管")&string_next.contains("胰腺")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>0.5) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>0.5) string_next else result_vue
      }

      if(string_next.contains("胆囊")&string_next.contains("息肉")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>10) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>10) string_next else result_vue
      }

      if(string_next.contains("肝脏")&string_next.contains("血管瘤")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>10) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>10) string_next else result_vue
      }

      if(string_next.contains("肝脏")&string_next.contains("囊肿")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>10) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>10) string_next else result_vue
      }

      if(string_next.contains("颈")&string_next.contains("狭窄")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>50) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>50) string_next else result_vue
      }

      if(string_next.contains("胆囊")&string_next.contains("息肉")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>0.8) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>0.8) string_next else result_vue
      }

      if(string_next.contains("血管瘤")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>5) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>5) string_next else result_vue
      }

      if(string_next.contains("血管瘤")&string_next.contains("肝")){
        val max: String = getCm(string_next)
        //判断
        result_status = if(max.toDouble>3) "0" else result_status
//        println(string_next)
        result_vue=if(max.toDouble>3) string_next else result_vue
      }

      if(string_next.contains("宫颈刮片")&(string_next.contains("III")|string_next.contains("IV")|string_next.contains("V"))){
        //判断
        result_status = "0"
//        println(string_next)
      }

      if(string_next.contains("心动过速")){
        val max: String = getCm(string_next)
//        println(string_next)
        //判断
        result_status = if(max.toDouble>=150) "0" else result_status
        //        println(string_next)
        result_vue=if(max.toDouble>=150) string_next else result_vue
      }

    }

    if(result_status.equals("1")){
      "1"
    }else{
      "0,"+result_vue
    }


//    regex.findAllIn()
  }


  /*def main(args: Array[String]): Unit = {
    val Match= noNumMatch("可疑外阴肿物体征")
    val max_cm: String = numMatch(""" 50.519.60体重指数:19.60
      血压:103/67mmHg子宫肌瘤5mm
      腰围(cm):70双侧乳腺多发实性占位，右侧最大约：0.7x0.4cm，左侧最大约1.2x0.6cm：纤维瘤？（建议定期复查）双侧腋窝查见副乳（右侧厚约1.5cm，左侧厚约0.8cm）。
    臀围(cm):87103708767160.511.8体脂率测定正常范围, 标准型未见异常正常未见溃疡及肿物幽门：圆，开闭好十二指肠球部：可见片状溃疡，直径约3.4cm，底覆白苔异常内科检查未发现明显异常整齐68未见异常正常剖宫产术后未见异常未见异常未见异常未见异常未见异常未见异常未见异常未见异常详见纸质报告单部位核磁共振成像未发现明显异常颈椎序列正常，生理曲度存在，各椎体形态、大小正常，未见明显异常信号，各椎间盘信号未见明显异常，未见明显突出或膨隆征象，后纵韧带及黄韧带未见增厚，椎管无狭窄。颈髓形态、大小正常，其内未见明显异常信号。周围软组织未见异常信号。无牙列不齐, 左右下后智齿, 左右下后阻生齿未见明显异常牙龈:牙龈炎
    龋齿:左上后残根
    牙周:牙结石宫颈囊肿数个，较大约0.6*0.7cm左侧卵巢囊肿约2.7*3.0cm，内透声差
    其他:牙列不齐,左右下后智齿,左右下后阻生齿室性心动过速，心室率1500次/min
    | 胆囊息肉(4.5mmx4mm) 建议随访肝、胰、脾、左肾、右肾未发现明显异常
    余口腔检查未见异常结肠溃疡.*息肉gggg0.2cm牙结石无牙龈炎无未见明显异常未见明显异常左上后残根双侧乳腺小叶增生无未见异常未见异常，活动自如双侧乳腺小叶增生
    余外科检查未见异常无未见异常无未见异常无无无未见异常无未见异常，活动自如阴性阴性黄色未见未见阴性透明1.015阴性未见未见阴性阴性阴性阴性阴性阴性6.0双侧乳腺小叶增生
      双侧乳腺导管囊状扩张双侧乳腺腺体层增厚,内部结构紊乱,回声不均，未见异常血流。双侧乳腺腺体层内均见管状无回声区，右侧最宽约0.23cm，左侧最宽约0.2cm。（BI-RADS：2类）彩超（甲状腺）未发现明显异常甲状腺形态大小正常，边界清晰，内部回声分布均匀，未见明显异常回声。（TI-RADS：1类）右肾大小、形态正常，包膜光滑，肾实质回声均匀，集合系统未见明显分离。脾:脾内钙化灶
    肝、胆、胰、左肾、右肾未发现明显异常左肾大小、形态正常，包膜光滑，肾实质回声均匀，集合系统未见明显分离。肝脏大小、形态正常，包膜光滑，肝内光点分布均匀，肝内管道系统清晰，肝内外胆管无扩张；门脉不宽。胆囊大小正常，壁光滑，腔内暗区清晰，胆总管测值正常范围。胰腺头、体、尾大小测值正常，内回声均匀。脾脏形态大小正常，包膜光整，实质回声均匀，脾内可见一直径约0.5cm的点状强回声，声影不明显。窦性心律
    正常心电图1.333.75849.27 ↑(建议定期复查！)3.0218.4324.41 ↑(建议定期复查！)113.946.1 ↑(建议定期复查！)双屈光不正右眼压检测正常眼球:双眼球突出
   | 其它:双屈光不正,不规则阴道出血左肾囊肿约1.5*1.2cm肾脏实质变薄，回声增强
    中下腹部偏右侧可见巨大软组织密度肿块，最大层面约13.3cm×8.8cm，
    腹部巨大占位性病变小约11.2mmx3.7mm。位于颈内动脉起始段。左侧颈内动脉起始段狭窄，狭窄面积约60%。
     子宫形态大小正常，轮廓光整，子宫内膜厚径2mm，回声均匀。肌层回声均匀，宫颈内可见多个无回声团块，较大约10mm×7mm，上述团块边界清晰，内
    余眼科检查未见异常左眼压检测正常正常正常正常双眼球突出正常正常1.51.0正常色觉：正常。正常正常2.105.14未见异常未见异常未见异常未见异常未见异常耳鼻喉检查未发现明显异常未见异常未见异常未见异常未见异常未见异常未见异常无未见异常1.1812.813.3815.8213 ↑3.6376.131.391.4244.711.4454.7373312.19 ↓4.1223352.10.8036.862.431.5411.9726.2826.34.3183.070.9920.920.465.63胸部螺旋CT断层扫描未发现明显异常   两侧胸廓对称。肺窗示两肺纹理清晰，走行自然，未见明显异常密度影。两侧肺门不大。纵隔窗示心影及大血管形态正常，纵隔内未见肿块及明显肿大淋巴结。无胸腔积液及胸膜增厚。0.000.00.010.10.28148.4 ↓0.254.62.670.30.022.10.128.130.43202.4744.6 ↑5.5312.244.840.795.14.2815.6345 ↑(建议定期复查！)1304.772.123.610.671.5119.0301.230.800.99骨量正常
""")

    println(max_cm)
  }*/


}
